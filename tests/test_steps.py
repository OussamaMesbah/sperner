...
def test_the_hex_captions_follow_the_turns():
    ...
    for step in range(1, len(walk.path)):
        blue_ahead = walk.path[step][0] != walk.path[step - 1][0]
        assert ("turns left" in captions[step]) == blue_ahead
...
def test_the_turn_captions_need_only_the_pairs_of_a_walk():
    ...
    assert "turns left" in captions[1] and "turns right" in captions[2]
